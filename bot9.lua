...
while true do
    ...
    if pid_result == "" then
        ...
        -- ============================================================
        -- STRATEGI LAUNCH: 2 FASE
        -- Fase 1: Force-stop app lama, lalu launch via deep link (fullscreen)
        -- Fase 2: Pindahkan ke freeform window setelah app aktif
        -- ============================================================
        
        -- Fase 0: Bersihkan state lama
        os.execute("am force-stop " .. pkg .. " 2>/dev/null")
        os.execute("sleep 1")

        -- Fase 1: Launch app via deep link (MODE FULLSCREEN DULU)
        -- Flag 0x10000000 = FLAG_ACTIVITY_NEW_TASK (wajib saat launch dari non-Activity context)
        ...
        -- Tunggu app benar-benar muncul (polling, max 30 detik)
        ...
        if not app_started then
            print("⚠️ App belum terdeteksi setelah 30 detik.")
            
            -- Fallback: Coba launch dengan component name langsung
            if launch_component then
                print("🔄 Mencoba fallback dengan component name...")
                local fallback_cmd = string.format(
                    "am start -a android.intent.action.VIEW -d '%s' -f 0x10000000 -n %s 2>&1",
                    ps_link, launch_component
                )
                print("🔧 [DEBUG] Fallback: " .. fallback_cmd)
                local fb_result = execute_raw(fallback_cmd)
                print("🔧 [DEBUG] Result: " .. fb_result:gsub("\n", " "))
                os.execute("sleep 10")
            end
        end
        
        -- Fase 2: Coba pindahkan ke freeform (opsional, tidak blocking)
        -- Ambil task ID dari app yang baru diluncurkan
        local task_raw = execute_raw("am stack list 2>/dev/null | grep -i '" .. pkg .. "' | head -1")
        local task_id = task_raw:match("taskId=(%d+)")
        
        if task_id then
            print("🪟 Task ID: " .. task_id .. " - Mencoba pindah ke freeform...")
            ...
            if w_str and h_str then
                ...
                local right = math.floor(w * 3 / 4)
                local bottom = math.floor(h * 3 / 4)
                
                -- Coba resize task ke freeform bounds
                -- Metode 1: am task resize (tersedia di beberapa Android versi)
                local resize_cmd = string.format(
                    "am task resize %s %d %d %d %d 2>&1",
                    task_id, left, top, right, bottom
                )
                print("🔧 [DEBUG] Resize: " .. resize_cmd)
                local resize_result = execute_raw(resize_cmd)
                
                if resize_result:find("Error") or resize_result:find("Unknown") then
                    print("⚠️ am task resize tidak tersedia. Mencoba wm...")
                    -- Metode 2: Set windowing mode via am
                    os.execute("am stack set-windowing-mode " .. task_id .. " 5 2>/dev/null")
                end
                
                print("🪟 Freeform bounds: " .. left .. "," .. top .. "," .. right .. "," .. bottom)
            end
        else
            print("ℹ️ Tidak dapat menemukan task ID, app berjalan fullscreen.")
        end
