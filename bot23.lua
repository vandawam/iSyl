...
while true do
    ...
    if pid_result == "" then
        ...
        print("📱 [Launch] Membuka window Freeform...")
        local cmd1
        if launch_component then
            cmd1 = string.format("am start --windowingMode 5 -n %s 2>&1", launch_component)
        else
            -- Fallback jika component tidak ditemukan, mencoba menggunakan monkey
            cmd1 = string.format("monkey -p %s -c android.intent.category.LAUNCHER 1 2>&1", pkg)
            print("⚠️ Menggunakan monkey sebagai fallback (Mungkin tidak freeform)")
        end
