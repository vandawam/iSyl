...
while true do
    ...
    if pid_result == "" then
        ...
        -- ============================================================
        -- STRATEGI LAUNCH: FREEFORM + DEEP LINK
        -- Kunci: --windowingMode 5 + package name (TANPA -n)
        --   - -n (component) mem-bypass intent resolver → deep link gagal
        --   - package di akhir → Android pilih activity yang tepat untuk URL
        -- Setelah launch: tap otomatis agar window mendapat fokus
        -- ============================================================
        ...
        local screen_w = tonumber(w_str) or 720
        local screen_h = tonumber(h_str) or 1280
        print(string.format("📐 Resolusi: %dx%d", screen_w, screen_h))

        local app_started = false

        -- ===== METODE 1: Freeform + deep link via package name =====
        -- PENTING: Jangan pakai -n (component name)! 
        -- -n mem-bypass intent resolver sehingga deep link tidak diproses
        print("🪟 [Metode 1] Launch freeform + deep link...")
        ...
        -- ===== METODE 2: Fallback tanpa freeform =====
        if not app_started then
            print("📱 [Metode 2] Fallback: launch biasa + deep link...")
            local start_cmd = string.format(
        ...
        -- ===== POST-LAUNCH: Tap untuk memberi fokus pada freeform window =====
        if app_started then
            print("👆 Mengirim tap ke tengah layar agar window mendapat fokus...")
            os.execute("sleep 2")
            -- Tap di tengah layar (dimana freeform window berada)
            local tap_x = math.floor(screen_w / 2)
            local tap_y = math.floor(screen_h / 2)
            os.execute(string.format("input tap %d %d", tap_x, tap_y))
            os.execute("sleep 1")
            -- Tap lagi untuk memastikan (double tap kadang diperlukan)
            os.execute(string.format("input tap %d %d", tap_x, tap_y))
            print(string.format("👆 Tap dikirim ke (%d, %d)", tap_x, tap_y))
        end
