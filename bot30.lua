...
while true do
    ...
    if pid_result == "" then
        ...
        -- ===== TAHAP 1: Launch Freeform Natively =====
        print("📱 [Launch] Membuka window Freeform secara Native...")
        
        -- Kita gunakan am start murni (tanpa --bounds karena Android 10 tidak support)
        local cmd1
        if launch_component then
        ...
        -- Beri jeda agar sistem membuat Task-nya terlebih dahulu
        os.execute("sleep 2")
        
        -- Trik Seamless: Panggil am start sekali lagi TANPA windowingMode
        -- Ini akan memaksa Android membawa Task Freeform yang tersembunyi/di background tadi ke depan!
        if launch_component then
            print("📱 [Launch] Memaksa window ke depan layar...")
            execute_raw(string.format("am start -n %s 2>&1", launch_component))
        end
        ...
        if app_started then
            ...
            -- Sertakan --windowingMode 5 agar jika link memaksa task baru, tetap dalam freeform
