...
while true do
    ...
    if pid_result == "" then
        ...
        -- ============================================================
        -- STRATEGI LAUNCH: FULLSCREEN (Freeform tidak didukung Redfinger)
        -- Metode 1: Deep link via component name (-n) → paling akurat
        -- Metode 2: Deep link via package name → fallback
        -- ============================================================
        ...
        -- ===== METODE 1: Launch via component name (deep link) =====
        if launch_component then
            print("📱 [Metode 1] Launch via component name...")
            local start_cmd = string.format(
                "am start -n %s -a android.intent.action.VIEW -d '%s' 2>&1",
        ...
        -- ===== METODE 2: Launch via package name (fallback) =====
        if not app_started then
            print("📱 [Metode 2] Launch via package name...")
            local start_cmd = string.format(
