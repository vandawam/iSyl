...
while true do
    ...
    if pid_result == "" then
        ...
        -- ===== TAHAP 1: Launch Freeform Window via am start =====
        print("📱 [Launch] Membuka window Freeform di depan...")
        local cmd1
        if launch_component then
            cmd1 = string.format("am start --windowingMode 5 -n %s 2>&1", launch_component)
        else
            cmd1 = string.format("monkey -p %s -c android.intent.category.LAUNCHER 1 2>&1", pkg)
            print("⚠️ Menggunakan monkey sebagai fallback")
        end
        ...
        if app_started then
            ...
            -- Ambil Task ID dari dumpsys activity
            local task_id = nil
            for line in execute_raw("dumpsys activity activities 2>/dev/null"):gmatch("[^\r\n]+") do
                if line:find(pkg) then
                    -- Format 1: Hist #0: ActivityRecord{... u0 com.pkg/... t123}
                    task_id = line:match(" t(%d+)%}")
                    if not task_id then
                        -- Format 2: ... taskId=123 ...
            ...
            if task_id then
                print(string.format("📐 [Freeform] Mengatur ukuran Task ID: %s", task_id))
                local resize_cmd = string.format("am task resize %s %d %d %d %d 2>&1", task_id, left, top, right, bottom)
                print("🔧 [DEBUG] Resize Cmd: " .. resize_cmd)
                execute_raw(resize_cmd)
            else
                print("⚠️ Gagal mendapatkan Task ID untuk di-resize.")
            end
