...
while true do
    ...
    if pid_result == "" then
        ...
        -- Tahap 1: Memaksa launch window Freeform menggunakan Component Name (-n)
        -- Tahap 2: Setelah window terbuka, inject deep link ke window tersebut.
        -- ============================================================
        ...
        -- Cari component name (Main Activity)
        local component = get_launch_activity(pkg)

        -- ===== TAHAP 1: Launch Freeform Window =====
        print("📱 [Tahap 1] Membuka window Freeform...")
        local cmd1 = ""
        if component then
            cmd1 = string.format("am start --windowingMode 5 -n %s", component)
        else
            cmd1 = string.format("am start --windowingMode 5 %s", pkg)
        end
        print("🔧 [DEBUG] Command 1: " .. cmd1)
        execute_raw(cmd1)

        -- Tunggu app aktif sebentar agar window terbuat
        print("⏳ Menunggu window spawn...")
        for i = 1, 5 do
            os.execute("sleep 2")
