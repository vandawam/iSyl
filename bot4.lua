...
if execute_command("id -u") ~= "0" then
    ...
    -- Mengambil path absolut dari skrip ini agar tsu tidak gagal menemukan file
    local script_path = execute_command("realpath " .. arg[0])
    
    -- Menjalankan ulang skrip dengan root, mengikat input terminal (< /dev/tty)
    -- untuk mencegah io.read() mengalami freeze
    os.execute("tsu -c 'lua " .. script_path .. "' < /dev/tty")
    os.exit()
end
