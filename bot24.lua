...
while true do
    ...
    if pid_result == "" then
        ...
        local cmd1
        if launch_component then
            cmd1 = string.format("am start --windowingMode 5 --bounds %s -n %s 2>&1", bounds, launch_component)
        else
        ...
        if app_started then
            ...
            local cmd2 = string.format(
                "am start --windowingMode 5 --bounds %s -a android.intent.action.VIEW -d '%s' -f 0x10000000 %s 2>&1",
                bounds, ps_link, pkg
            )
