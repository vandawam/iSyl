...
while true do
    ...
    if pid_result == "" then
        ...
        -- ===== TAHAP 1: Launch Normal =====
        print("📱 [Launch] Membuka aplikasi secara normal...")
        local cmd1
        if launch_component then
            cmd1 = string.format("am start -n %s 2>&1", launch_component)
        else
        ...
        if app_started then
            ...
            -- ===== TAHAP 1.5: Out of the box UI Automation to Freeform =====
            print("🤖 Menggunakan metode Out-of-the-Box (UI Automation) untuk Freeform...")
            convert_to_freeform(pkg)
